...
type IUserTokenDo interface {
	...
	FilterWithToken(token string) (result []model.UserToken, err error)
}

// SELECT * FROM @@table WHERE token = @token{{end}}
func (u userTokenDo) FilterWithToken(token string) (result []model.UserToken, err error) {
	var params []interface{}

	var generateSQL strings.Builder
	params = append(params, token)
	generateSQL.WriteString("SELECT * FROM user_tokens WHERE token = ? ")
